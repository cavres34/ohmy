...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001915718534, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="🇹🇷 ʙᴇɴɪ ɢʀᴜʙᴀ ᴇᴋʟᴇ 🇹🇷", url="https://t.me/oyungametelegram")
            callback_button2 = types.InlineKeyboardButton(text="⚙️ ʀᴇsᴍɪ ᴋᴀɴᴀʟ ⚙️", url="https://t.me/oyungametelegram")
