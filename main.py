...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001984404074, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def sessiz_sinema_baslat(message, **kwargs): 
    ...
    callback_button = types.InlineKeyboardButton(text="Kendi Kelimem 📝", url=f"")
